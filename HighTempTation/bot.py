...
_dashboard_dir = os.path.join(_bot_dir, 'dashboard')
if _dashboard_dir not in sys.path:
    sys.path.insert(0, _dashboard_dir)
# 根目录无条件放在最前: 确保新生产级模块 (calibrator/cost_model/portfolio_risk/api_server)
# 优先于 dashboard/ 下的旧版同名模块。注意: 脚本目录天然在 sys.path 中但位置靠后,
# 必须显式 insert(0) 才能压过 dashboard。
...
# ── 生产级架构模块 (Day1) ────────────────────────────────────────────
# 概率校准 (Isotonic Regression) + 交易成本模型 + 组合风控 + FastAPI
try:
...
# ── 生产级全局对象 (供 api_server /api/metrics 读取) ──
_calibrator = None        # ProbabilityCalibrator
_cost_model = None        # CostModel
...
class Config:
    def __init__(self):
        ...
        # === 🎯 生产级: 概率校准 (Isotonic Regression, Day1 #1) ===
        self.CALIB_ENABLED = os.getenv("CALIB_ENABLED", "true").lower() == "true"
        self.CALIB_RETRAIN_DAILY = os.getenv("CALIB_RETRAIN_DAILY", "true").lower() == "true"
        # 校准应用的信号类型 (逗号分隔; ALL=全部)
        self.CALIB_APPLY_TO = os.getenv("CALIB_APPLY_TO", "MODEL,LADDER").upper()

        # === 💸 生产级: 交易成本模型 (Day1 #2) ===
        # 细参数 (COST_TAKER_FEE / COST_GAS_PER_SHARE / ...) 在 cost_model.py 内读取
        self.COST_ENABLED = os.getenv("COST_ENABLED", "true").lower() == "true"
        self.MIN_NET_EDGE = float(os.getenv("MIN_NET_EDGE", "0.03"))  # 最小净 edge

        # === 🛡️ 生产级: 组合风控 (Day1 #5) ===
        # 细参数 (RISK_MAX_POS_PER_EVENT / RISK_CORR_BREAKER_*) 在 portfolio_risk.py 内读取
        self.RISK_ENABLED = os.getenv("RISK_ENABLED", "true").lower() == "true"
...
class WeatherClient:
    ...
    async def get_city_forecasts(self, city_list: Optional[list[tuple[str, float, float]]] = None) -> list[dict]:
        """
        获取指定列表城市的预报 (生产级: asyncio.gather 并行拉取)。

        city_list: [(city_name, lat, lon), ...]，None 则用 STATION_IDX
        并发控制: 信号量 SEMAPHORE (默认 10) 防止打爆 Open-Meteo API。
        返回 [{
        ...
        # 预处理: 过滤无效坐标, 收集 (city, lat, lon, icao)
        # 传入坐标优先 (调用方可能已 geocode 新城市); 无效时回退站表
        valid = []
        for item in sources:
            if len(item) == 3:
                city, lat, lon = item
            ...
            if (lat == 0 and lon == 0) and city in STATION_IDX:
                lat, lon = station_coords(city)
            if lat == 0 and lon == 0:
                logger.debug(f"跳過 {city}: 無效坐標")
                continue
            valid.append((city, lat, lon, icao))

        # 并发拉取 (Day1 #3): asyncio.gather 并行请求所有城市,
        # 单城市内多日预报也并行; 信号量限流
        sem = asyncio.Semaphore(max(4, min(16, len(valid) // 4 + 4)))

        async def _fetch_one(city: str, lat: float, lon: float, icao: str) -> list[dict]:
            """拉取单城市预报并做偏差校正 (与原串行逻辑等价)"""
            async with sem:
        ...
        # 并行拉取全部城市
        tasks = [_fetch_one(c, la, lo, ic) for c, la, lo, ic in valid]
        gathered = await asyncio.gather(*tasks, return_exceptions=True)

        # 展平 + 保持输入顺序 + 统一日志 (顺序与原串行版一致)
        results: list[dict] = []
        for i, (city, lat, lon, icao) in enumerate(valid):
            recs = gathered[i]
            if isinstance(recs, Exception):
                logger.warning(f"預報失敗 {city}: {recs}")
                continue
            for rec in recs:
                results.append(rec)
                bias_info = f" 校正{rec['bias_correction']:+.1f}°C" if rec["bias_correction"] else ""
                label = icao or f"({lat},{lon})"
                logger.info(f"🌤️  {city}({label}) {rec['date']}: μ={rec['mean']:.1f}°C σ={rec['sigma']:.1f}°C ({rec['model_count']}模型){bias_info}")

        if len(valid) > 1:
            logger.info(f"  ⚡ 并行預報: {len(valid)} 城市并发拉取完成 (gather)")
        return results
...
class Engine:
    ...
    def calc_signal_size(self, sig: dict) -> float:
        """
        统一仓位计算 (生产级): 开仓循环与组合风控共用同一口径。

        链路: Kelly/旧公式 → 动态预算 → 阶梯缩放 → 取整。
        """
        sig_type = sig.get("signal_type", "MODEL")
        if cfg.KELLY_ENABLED:
            base_size = self.calc_kelly_size(
                sig.get("p_model", 0.5), sig.get("p_market", 0.5),
                sig.get("side", "NO"), sig_type)
        else:
            base_size = self._legacy_position_size(sig.get("diff", 0))

        # v7: 动态预算 (基于历史胜率和盈亏比)
        if cfg.DYNAMIC_BUDGET_ENABLED:
            dyn = self.calc_dynamic_budget(sig.get("city", ""))
            if dyn < cfg.POS_MIN:
                dyn = cfg.POS_MIN
            if dyn < base_size:
                base_size = dyn
...
async def main():
    ...
    global _calibrator, _cost_model, _portfolio_risk, _last_calib_retrain_date
    ...
    # 🎯 生产级: 概率校准 + 成本模型 + 组合风控
    logger.info(f"  🎯 概率校准={'ON' if cfg.CALIB_ENABLED else 'OFF'} "
                f"应用类型={cfg.CALIB_APPLY_TO} 每日重训={'ON' if cfg.CALIB_RETRAIN_DAILY else 'OFF'}")
    logger.info(f"  💸 成本模型={'ON' if cfg.COST_ENABLED else 'OFF'} "
                f"MIN_NET_EDGE={cfg.MIN_NET_EDGE:.1%} "
                f"(edge_net = |p_model-p_market| - fee - gas - slippage - impact - θ)")
    logger.info(f"  🛡️ 组合风控={'ON' if cfg.RISK_ENABLED else 'OFF'} "
                f"(同事件/相邻桶仓位限制 + 相关性熔断, 详见 /api/metrics)")
    logger.info(f"  🌐 API: FastAPI 自动文档 /docs + 生产指标 /api/metrics + /api/calibration")
    # 🧠 v8: 贝叶斯实时概率更新
    ...
    # ── 生产级架构模块 (Day1) ──
    # 🎯 概率校准: 加载历史模型 + 从已结算信号重训 (Isotonic Regression)
    _calibrator = None
    if cfg.CALIB_ENABLED and ProbabilityCalibrator is not None and _engine.db:
        try:
            _calibrator = ProbabilityCalibrator(db=_engine.db)
            _calibrator.load_all()
            trained = _calibrator.retrain()
            if trained:
                logger.info(f"🎯 概率校准器就绪: 全局+{len(_calibrator._models)} 城市模型 "
                            f"(ECE={_calibrator._metrics.get('_global', {}).get('ece', 'N/A')})")
            else:
                logger.info(f"🎯 概率校准器加载完成 (样本不足, 使用原始概率回退)")
        except Exception as e:
            logger.warning(f"🎯 概率校准器初始化失败: {e}")
            _calibrator = None
    else:
        logger.info(f"🎯 概率校准=OFF (CALIB_ENABLED={cfg.CALIB_ENABLED})")

    # 💸 交易成本模型
    _cost_model = CostModel() if cfg.COST_ENABLED and CostModel is not None else None
    if _cost_model:
        logger.info(f"💸 成本模型就绪: MIN_NET_EDGE={_cost_model.min_net_edge:.1%} "
                    f"fee={_cost_model.taker_fee:.1%} gas={_cost_model.gas_per_share:.1%}/股")
    else:
        logger.info(f"💸 成本模型=OFF (COST_ENABLED={cfg.COST_ENABLED})")

    # 🛡️ 组合风控
    _portfolio_risk = PortfolioRiskManager() if PortfolioRiskManager is not None else None
    if _portfolio_risk:
        logger.info(f"🛡️ 组合风控就绪: 同事件≤{_portfolio_risk.max_pos_per_event}笔/"
                    f"${_portfolio_risk.max_exposure_event:.0f} 相邻桶≤{_portfolio_risk.max_adjacent_buckets} "
                    f"熔断≥{_portfolio_risk.corr_breaker_count}信号/"
                    f"{_portfolio_risk.corr_breaker_cooldown}s")
    _last_calib_retrain_date = date.today().isoformat()
    ...
    # FastAPI 服务器 (替代手写 http.server, 自动 /docs + /api/metrics)
    ...
    try:
        while running:
            ...
            # ── 🎯 概率校准每日重训 (Day1 #1 闭环: 已结算信号 → 重训模型) ──
            if _calibrator and cfg.CALIB_ENABLED and cfg.CALIB_RETRAIN_DAILY:
                if _last_calib_retrain_date != date.today().isoformat():
                    _last_calib_retrain_date = date.today().isoformat()
                    logger.info("🎯 每日校准重训开始...")
                    try:
                        _calibrator.retrain()
                    except Exception as e:
                        logger.warning(f"🎯 校准重训失败: {e}")
            ...
            # 生产级: 信号量限流 (Semaphore 5) 防止 Open-Meteo 429 (并行 burst 限流)
            ensemble_40_index = {}  # city_date → {date, members, mu, sigma,...}
            ens40_fetch_count = 0
            if cfg.ENSEMBLE_40_ENABLED and cfg.ENSEMBLE_40_MODE != "deterministic_only":
                ens40_sem = asyncio.Semaphore(5)
                async def _ens40_one(c: str, lat: float, lon: float):
                    async with ens40_sem:
                        return c, await _weather.get_ensemble_40_forecast(lat, lon)
                ens40_tasks = [_ens40_one(c, lat, lon) for c, lat, lon in city_forecast_list]
                ens40_results = await asyncio.gather(*ens40_tasks, return_exceptions=True)
                for ci, result in enumerate(ens40_results):
                    if isinstance(result, Exception):
                        logger.debug(f"Ensemble40 {city_forecast_list[ci][0]}: {result}")
                        continue
                    city, day_list = result
                    for day in day_list:
            ...
            for m in markets:
                ...
                # ── 🎯 概率校准 (Day1 #1): 高斯 CDF 原始概率 → 实际频率 ──
                # 只校准 MODEL 系信号 (METAR/HKO/BAYESIAN 概率来源不同, 默认不校准)
                p_model_raw = p_model
                if _calibrator and cfg.CALIB_ENABLED:
                    apply_set = {t.strip().upper() for t in cfg.CALIB_APPLY_TO.split(",")}
                    if "ALL" in apply_set or "MODEL" in apply_set:
                        p_model_cal = _calibrator.calibrate_prob(p_model, parsed["city"])
                        if abs(p_model_cal - p_model) > 0.001:
                            logger.debug(f"  🎯 校准 [{parsed['city']} {parsed['label']}]: "
                                         f"{p_model:.1%} → {p_model_cal:.1%}")
                            p_model = p_model_cal
            ...
            if cfg.LADDER_ENABLED and signals:
                for sig in signals:
                    ...
                    for side in range(-actual_spread, actual_spread + 1):
                        ...
                        # 用相同 mu/sigma 计算相鄰桶的概率
                        p_model_adj = bucket_prob(lo, hi, sig["mu"], sig["sigma"])
                        # 🎯 阶梯桶同样过概率校准 (若启用)
                        p_model_adj_raw = p_model_adj
                        if _calibrator and cfg.CALIB_ENABLED:
                            apply_set = {t.strip().upper() for t in cfg.CALIB_APPLY_TO.split(",")}
                            if "ALL" in apply_set or "LADDER" in apply_set:
                                p_model_adj = _calibrator.calibrate_prob(p_model_adj, city)
                        ...
                        if is_no_signal and "NO" in cfg.ALLOWED_SIDES:
                            ladder_sig = {
                                ...
                                "p_model_raw": round(p_model_adj_raw, 4),
                                ...
                                "liquidity": float(adj_market.get("liquidity", 0) or 0),
                                "signal_type": "LADDER",
                            }
            ...
            if not Engine.in_trading_window():
                ...
                # ── 💸 生产级预过滤: 交易成本模型 (Day1 #2) + 组合风控 (Day1 #5) ──
                # 先统一计算计划仓位 (与开仓同口径)，供成本/风控评估
                pre_signals = []
                cost_rejected = 0
                for sig in signals:
                    size_est = _engine.calc_signal_size(sig)
                    sig2 = dict(sig)
                    sig2["planned_size"] = size_est
                    if _cost_model and cfg.COST_ENABLED:
                        cb = _cost_model.evaluate_signal(sig2, size_est)
                        if not cb.is_profitable:
                            cost_rejected += 1
                            logger.info(f"  💸 成本过滤 [{sig2['city']} {sig2['bucket']}] "
                                        f"{sig2['side']}: {cb.detail}")
                            continue
                    pre_signals.append(sig2)
                if cost_rejected:
                    logger.info(f"  💸 成本模型: {cost_rejected} 个信号净 edge 不足被过滤")

                # 组合风控: 同事件限制 + 相邻桶限制 + 相关性熔断
                if _portfolio_risk and cfg.RISK_ENABLED and pre_signals:
                    risk_allowed, risk_rejected, breakers = _portfolio_risk.filter(
                        pre_signals, _engine, cfg)
                    if risk_rejected:
                        logger.info(f"  🛡️ 组合风控: 拒绝 {len(risk_rejected)} 个信号 "
                                    f"(放行 {len(risk_allowed)})")
                    pre_signals = risk_allowed
                ...
                for sig in pre_signals:
                    ...
                    # ── 🎯 仓位: 预过滤阶段已统一计算 (planned_size), 保证与风控口径一致 ──
                    size = sig.get("planned_size", 0.0) or 0.0
                    ...
                    if pos is not None:
                        # 记录原始模型概率 (校准前) 用于信号历史 → 校准训练数据
                        pos._p_model = sig.get("p_model_raw") or sig.get("p_model")
                        if is_ladder:
                            ladder_opened += 1
                        else:
                            opened += 1

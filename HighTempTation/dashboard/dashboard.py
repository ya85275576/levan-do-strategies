...
def fetch_trades_data(limit: int = 100) -> dict:
    """从 /api/trades 获取历史交易记录"""
    try:
        url = f"{API_TRADES_URL}?limit={limit}"
...
def render_historical_trades():
    """从 TradeDB 读取并展示历史交易记录"""
    raw = fetch_trades_data(limit=200)
    ...
    if trades:
        ...
        st.caption(f"📊 历史统计: {len(trades)}笔已平仓 | 胜率 {win_rate:.1f}% ({wins}胜/{losses}负) | 累计盈亏 ${total_pnl:+.2f}")

    # ── 当前持仓（DB 持久化） ──
    if open_trades:
        st.subheader("📋 DB 持仓记录")
        df_open = pd.DataFrame(open_trades)
        open_cols = ["city", "side", "entry_price", "size", "entry_time"]
        available_open = [c for c in open_cols if c in df_open.columns]
        st.dataframe(df_open[available_open], use_container_width=True, height=150)

    # ── 历史平仓表格 ──
    st.subheader("📜 全部历史交易记录")
    if trades:
        df = pd.DataFrame(trades)
        # 格式化列名：DB 字段 → 友好的中文列名
        col_map = {
        ...
        display_cols = ["entry_time", "city", "side", "entry_price", "exit_price", "pnl", "size", "exit_reason", "exit_time"]
        available_display = [c for c in display_cols if c in df.columns]
        df_display = df[available_display].copy()
        # 重命名
        rename_map = {k: v for k, v in col_map.items() if k in df_display.columns}
        ...
        try:
            df_plot = df.copy()
            df_plot["pnl"] = pd.to_numeric(df_plot["pnl"], errors="coerce").fillna(0)
            df_plot["cum_pnl"] = df_plot["pnl"].cumsum()
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=list(range(len(df_plot))),
                y=df_plot["cum_pnl"].values,
                mode="lines",
                name="累计盈亏",
            ...
            fig.add_trace(go.Bar(
                x=list(range(len(df_plot))),
                y=df_plot["pnl"].values,
                name="单笔盈亏",
                marker_color=df_plot["pnl"].apply(lambda x: "#2ea043" if x >= 0 else "#da3633"),
                opacity=0.6,
            ))
            fig.update_layout(
                title="历史盈亏分布 (按交易序号)",

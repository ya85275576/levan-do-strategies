...
| # | 来源（服务器） | 开源仓库内文件 | 说明 |
|---|----------------|----------------|------|
| 1 | `HighTempTation/bot.py`（最新 v8，190KB） | `bot.py` | 天气 Bot 主程序：多源气象数据、概率模型、温度阶梯套利、Kelly 仓位、风控 |
| 2 | `services/webhook/dashboard.html` | `dashboard_forecast.html` | 天气预报面板（城市预报网格 / DB 状态） |
...
```bash
cd /root/levan-do-strategies/HighTempTation
# 同步最新 bot.py / dashboard.html / dashboard/ 等
git add -A && git commit -m "sync: 描述"
git push https://x-access-token:$GITHUB_PAT@github.com/ya85275576/HighTempTation.git main
```

注意：推送前须确保同步的文件不含密钥与内部引用（对照上文合规性检查清单）。
